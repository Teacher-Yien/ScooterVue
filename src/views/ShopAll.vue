      ...
      />
			...
			<div v-if="!loading && !error">
					...
					<div v-if="viewMode === 'grid'" class="row">
							...
							>
							<ProductGridItem @add-to-cart="handleAddToCart" :product="product" />
...
const filteredResults = computed(() => {
  ...
  return [...filterItems(products.value), ...filterItems(accessories.value)];
});
// Computed Property: Filter and Search Logic
const filteredListResults = computed(() => {
  ...
  return [...filterItems(filteredProducts.value), ...filterItems(filteredAccessories.value)];
});
...
const sortProducts = () => {
  ...
  if (sortFunction) {
    products.value.sort(sortFunction);
    accessories.value.sort(sortFunction);
				filteredProducts.value.sort(sortFunction);
...
const fetchProducts = async () => {
  ...
  try {
    ...
    filteredProducts.value = dataProducts.slice(0,6);
